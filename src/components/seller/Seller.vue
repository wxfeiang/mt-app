      ...
      .then(response => {
        ...
        this.$nextTick(() => {
          ...
          if (this.seller.poi_env.thumbnails_url_list) {
            let imgW = this.$refs.picsItem[0].clientWidth; // 单个图片的宽度
            let marginR = 11;
            let width =
              (imgW + marginR) * this.seller.poi_env.thumbnails_url_list.length; // 当前整个图片的宽度
            this.$refs.picsList.style.width = width + "px"; 
            this.scroll = new BScroll(this.$refs.picsView, {
              scrollX: true  // 横向滚动
            });
          }
             this.sellerView = new BScroll(this.$refs.sellerView);  // 滚动整屏
        });
      });
  }
};
